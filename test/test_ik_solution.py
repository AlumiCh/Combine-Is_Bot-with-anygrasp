...
# ===== 验证代码 =====
def verify_ik_solution(ik_solver, target_pos, target_quat, target_qpos, actual_qpos):
    """
    对比 IK 求解的准确性
    ...
    logger.info(f"  期望欧拉角(度): {target_euler.round(2)}")
    logger.info(f"  FK 欧拉角(度):   {fk_euler_target.round(2)}")
